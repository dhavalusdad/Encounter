...
import { useResetQueries } from '@encounter/common/hooks';
import {
...
import AdvancedSelect, { type SelectProps } from '.';
...
> extends SelectProps<OptionData, IsMulti, Group> {
  ...
  portalRootId?: string; // Portal container ID
    ...
    const appendOptions = async () => {
      ...
      try {
        ...
        if (nextPage?.data?.pages?.length) {
          ...
          let filteredOptions = [...options, ...flattenedData];
