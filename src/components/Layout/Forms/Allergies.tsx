...
const Allergies = () => {
  ...
  const { handleSubmit, setValue, getValues, reset, register } = useForm<
    (typeof form)['allergies']
  >({
    mode: 'onChange',
    defaultValues: form['allergies']
  });

  const currentForm = tabArray.find((el) => el.id === 'allergies');
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-4">
        ...
        <Button
          onClick={() => {
            ...
            dispatchUpdateEncounterValue({ allergies: defaultValue });
        ...
        <Button
          onClick={handleSubmit((value) => {
            dispatchUpdateEncounterValue({ allergies: value })
